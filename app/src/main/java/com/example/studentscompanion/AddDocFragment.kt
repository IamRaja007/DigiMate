...
import com.example.studentscompanion.response.CreateDocResponse
import com.example.studentscompanion.response.CreateDocWithPdfResponse
...
import kotlinx.android.synthetic.main.fragment_add_doc.*
import kotlinx.android.synthetic.main.fragment_add_query.*
import kotlinx.android.synthetic.main.fragment_add_query.etQuery
...
class AddDocFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnCancelDoc.setOnClickListener {
            activity?.onBackPressed()
        }

        btnCreateDoc.setOnClickListener {
            val textQuery=etQueryDoc.text.toString()
            ...
            if(requesttext != null && fileCheck!=null && fileType?.contains("image/*") == true){
                ...
                val upload=RetrofitBuilder.apiService.createDocWithTextPhoto(auth!!,bodyOfFile,requesttext,requestTime)
                upload.enqueue(object :retrofit2.Callback<CreateDocResponse>{

                    override fun onResponse(
                        call: Call<CreateDocResponse>,
                        response: Response<CreateDocResponse>
                    ) {
                        activity?.onBackPressed()
                ...
                val upload=RetrofitBuilder.apiService.createDocWithTextPdf(auth!!,bodyOfFile,requesttext,requestTime)
                upload.enqueue(object :retrofit2.Callback<CreateDocWithPdfResponse>{

                    override fun onResponse(
                        call: Call<CreateDocWithPdfResponse>,
                        response: Response<CreateDocWithPdfResponse>
                    ) {
                        etQueryDoc.text.clear()
                        activity?.onBackPressed()
                        println("upload success for both text and pdf")
                    }

                    override fun onFailure(call: Call<CreateDocWithPdfResponse>, t: Throwable) {
                ...
                val upload=RetrofitBuilder.apiService.createDocWithTextPhoto(auth!!,queryText = requesttext,queryTime = requestTime)
                upload.enqueue(object :retrofit2.Callback<CreateDocResponse>{
                ...
                val upload=RetrofitBuilder.apiService.createDocWithTextPhoto(auth!!,image = bodyOfFile,queryTime = requestTime)
                upload.enqueue(object :retrofit2.Callback<CreateDocResponse>{
                ...
                val upload=RetrofitBuilder.apiService.createDocWithTextPdf(auth!!,filePdf = bodyOfFile,queryTime = requestTime)
                upload.enqueue(object :retrofit2.Callback<CreateDocWithPdfResponse>{
...
//            }
        ...
        btnAddPhotoOrPdfDoc.setOnClickListener {
...
//
    ...
    private fun uploadImage(imageURI: Uri?) {
        ...
        if(imageURI!=null){
            fileCheck="Done"
            tvFileAttachedOrNotDoc.visibility=View.VISIBLE
        }
